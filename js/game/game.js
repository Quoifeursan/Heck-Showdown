(() => {
 ...
 /* J'ai modif */
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialiser les événements audio pour la page de jeu
    if (typeof eventManager !== 'undefined') {
      eventManager.initAudioButtonListeners();
    }
    
    // Démarrer la musique du jeu après que tous les scripts soient chargés
    if (typeof audioManager !== 'undefined') {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Boss de zone à la manche 200
if (r === 200 && state.player.zone && typeof BOSSES !== 'undefined' && BOSSES[state.player.zone]) {
  state.monster = JSON.parse(JSON.stringify(BOSSES[state.player.zone]));
  return;
}
...
/* J'ai modif */
    // Jouer le son approprié selon le type d'attaque
    if (typeof audioManager !== 'undefined') {
      if (type === 'special') {
        audioManager.playSpeelSound();
      } else if (type === 'real') {
        audioManager.playAttackSound();
      } else if (type === 'pure') {
        audioManager.playBrutSound();
      }
    }
    /* Jusqu'à la */
...
if (m.hp <= 0) {
  ...
  } else if (state.round <= 200) {
    if ((state.round - 1) % 20 === 0) checkpoint();
  }
...
// Modifiez la fonction openPassiveChoice() dans game.js

function openPassiveChoice() {
  const modal = $('passiveModal');

  // On filtre les passifs dispo pour la manche actuelle ET la zone
  const availablePassives = PASSIVES.filter(p => {
    // Vérification de la manche
    const roundCheck = (!p.unlockRound || p.unlockRound <= state.round) &&
                       (!p.maxRound || p.maxRound >= state.round);
    
    // Vérification de la zone
    const zoneCheck = !p.zones || 
                      (state.player.zone && p.zones.includes(state.player.zone)) ||
                      (!state.player.zone && p.zones.includes('inconnu'));
    
    return roundCheck && zoneCheck;
  });
  ...
  picks.forEach(p => {
    ...
    // Affichage de la zone si spécifiée
    const zoneInfo = p.zones ? `` : '';
